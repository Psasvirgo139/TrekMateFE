...
const BookingHistory = () => {
  ...
  // fetching = dữ liệu đang tải lần đầu hoặc tab mới (hiện skeleton)
  // refreshing = reload ngầm trang, giữ nội dung cũ, chỉ hiện spinner overlay
  ...
  // track previous tab để phân biệt tab switch vs page change
  ...
  useEffect(() => {
    ...
    const isTabSwitch = prevTabRef.current !== activeTab;
    prevTabRef.current = activeTab;

    // Tab switch → hiện skeleton, page change → overlay nhẹ
    if (isTabSwitch) {
      setFetching(true);
      setBookings([]);
    } else {
  ...
  return (
    <div className="bg-[#f7f9f6] min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {/* Skeleton loading — chỉ hiện lần đầu hoặc khi switch tab */}
        {fetching && (
        ...
        {/* Content area — bọc trong container tương đối để đặt overlay */}
        {!fetching && !error && (
          <div className="relative">
            {/* Overlay spinner nhẹ khi chuyển trang (không xóa content) */}
            {refreshing && (
              <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-10 flex items-center justify-center rounded-2xl">
                <div className="w-7 h-7 border-4 border-[#012d1d] border-t-transparent rounded-full animate-spin" />
              </div>
            )}
